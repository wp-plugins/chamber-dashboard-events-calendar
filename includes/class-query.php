...
new Cdash_Events_Query($cdash_events);

class Cdash_Events_Query
{
	...
	*/
	public function register_rewrite()
	{
		...
		if($this->options['general']['rewrite_rules'] === TRUE)
		{
			...
			update_option('cdash_events_general', $this->options['general']);
			flush_rewrite_rules();
		}
	}
		...
		if(is_admin())
		{
			...
			foreach($post_types as $post_type)
			{
				if($pagenow === 'edit.php' && isset($query->query_vars['post_type']) && $query->query_vars['post_type'] === $post_type)
				{
					...
					$sb = $cde_helper->is_valid_date($start);
					$eb = $cde_helper->is_valid_date($end);
	...
	*/
	...
	{
		if((is_tax('event-location') && isset($query->query_vars['event-location'], $query->query['event-location'])) || (is_tax('event-category') && isset($query->query_vars['event-category'], $query->query['event-category'])))
		{
			if(!isset($query->query_vars['event_show_occurrences']))
				$query->query_vars['event_show_occurrences'] = true;
			...
			if(!isset($query->query_vars['event_show_past_events']) || !is_bool($query->query_vars['event_show_past_events']))
				$query->query_vars['event_show_past_events'] = true;
		...
		if(is_array($post_types))
		{
			...
			if ((bool)array_diff($post_types, apply_filters('cde_event_post_type', array('event'))))
			{
			...
			$run_query = (bool)in_array($post_types, apply_filters('cde_event_post_type', array('event')));
		...
		if($run_query)
		{
			...
			if(!isset($query->query_vars['event_start_after']) || !($format_sa = $cde_helper->is_valid_datetime($query->query_vars['event_start_after'])))
				$query->query_vars['event_start_after'] = $defaults['event_start_after'];

			if(!isset($query->query_vars['event_start_before']) || !($format_sb = $cde_helper->is_valid_datetime($query->query_vars['event_start_before'])))
				$query->query_vars['event_start_before'] = $defaults['event_start_before'];

			if(!isset($query->query_vars['event_end_after']) || !($format_ea = $cde_helper->is_valid_datetime($query->query_vars['event_end_after'])))
				$query->query_vars['event_end_after'] = $defaults['event_end_after'];

			if(!isset($query->query_vars['event_end_before']) || !($format_eb = $cde_helper->is_valid_datetime($query->query_vars['event_end_before'])))
				$query->query_vars['event_end_before'] = $defaults['event_end_before'];

			if(!isset($query->query_vars['event_show_occurrences']))
				$query->query_vars['event_show_occurrences'] = (is_admin() ? false : true);
