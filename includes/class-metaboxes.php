...
new Cdash_Events_Metaboxes($cdash_events);

class Cdash_Events_Metaboxes
{
	...
	*/
	public function load_defaults()
	{
		$this->tickets_fields = apply_filters(
			'cde_event_tickets_fields',
			array(
				'name' => __('Ticket Name', 'cdash-events'),
				'price' => __('Price', 'cdash-events')
			)
		);
		...
		foreach ($post_types as $post_type)
		{
			$this->metaboxes[] = apply_filters(
				'cde_'.$post_type.'_metaboxes',
				array(
					'event-options-box' => array(
						'title' => __('Event Display Options', 'cdash-events'),
					...
					'event-date-time-box' => array(
						'title' => __('Event Date and Time', 'cdash-events'),
					...
					'event-cost-tickets-box' => array(
						'title' => __('Event Tickets', 'cdash-events'),
	...
	*/
	public function admin_scripts_styles($page)
	{
		...
		if(($page === 'post-new.php' || $page === 'post.php') && in_array($screen->post_type, apply_filters('cde_event_post_type', array('event'))))
		{
			global $wp_locale;

			wp_register_script(
				'cdash-events-datetimepicker',
			...
			{
				wp_register_script(
					'cdash-events-datetimepicker-localization',
					$lang_path,
					array('jquery', 'cdash-events-datetimepicker')
				);

				wp_enqueue_script('cdash-events-datetimepicker-localization');
			}

			wp_register_script(
				'cdash-events-admin-post',
				CDASH_EVENTS_URL.'/js/admin-post.js',
				array('jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'jquery-ui-slider', 'cdash-events-datetimepicker')
			);

			wp_enqueue_script('cdash-events-admin-post');
			...
			wp_enqueue_style('cdash-events-datetimepicker');
		}
	}
	...
	*/
	...
	{
		if (isset($post_type) && in_array($post_type, apply_filters('cde_event_post_type', array('event'))))
		{
	...
	*/
	...
	{
		wp_nonce_field('cdash_events_save_event_datetime', 'event_nonce_datetime');
		...
		<div class="date-time-row">
			<label for="event_start_date">'.__('Start date/time', 'cdash-events').':</label> <input id="event_start_date" type="text" name="event_start_date" value="'.esc_attr($event_start_date[0]).'"/> <input id="event_start_time" type="text" name="event_start_time" value="'.esc_attr(isset($event_start_date[1]) ? substr($event_start_date[1], 0, 5) : '').'" '.($event_all_day ? 'style="display: none;"' : '').'/>
		</div>
		<div class="date-time-row">
			<label for="event_end_date">'.__('End date/time', 'cdash-events').':</label> <input id="event_end_date" type="text" name="event_end_date" value="'.esc_attr($event_end_date[0]).'"/> <input id="event_end_time" type="text" name="event_end_time" value="'.esc_attr(isset($event_end_date[1]) ? substr($event_end_date[1], 0, 5) : '').'" '.($event_all_day ? 'style="display: none;"' : '').'/>
		</div>
		<div>
			<label for="event_all_day">'.__('All-day event?', 'cdash-events').'</label> <input id="event_all_day" type="checkbox" name="event_all_day" '.checked($event_all_day, true, false).'/>
		</div>
		<div class="date-time-row">
			<label for="event_recurrence">'.__('Recurrence', 'cdash-events').'</label> <select id="event_recurrence" name="event_recurrence[type]" class="">';
		...
		foreach($this->recurrences as $id => $recurrence)
		{
			...
			elseif($id === 'monthly')
			{
				...
				$options['monthly'] = '<input id="event_recurrence_day_month" type="radio" name="event_recurrence[monthly][monthly_day_type]" value="1"/ '.checked($day_type, 1, false).'><label for="event_recurrence_day_month">'.__('day of the month', 'cdash-events').'</label><br /><input id="event_recurrence_day_week" type="radio" name="event_recurrence[monthly][monthly_day_type]" value="2" '.checked($day_type, 2, false).'/><label for="event_recurrence_day_week">'.__('day of the week', 'cdash-events').'</label>';
			}
		...
		$template = '
		'.__('Repeat every', 'cdash-events').' <input type="text" size="2" maxlength="4" name="event_recurrence[repeat]" value="%1$d"/> <span class="occurrence">%2$s</span> '.__('until', 'cdash-events').' <input class="event_recurrence_until" type="text" name="event_recurrence[until]" size="10" maxlength="10" value="%3$s"/>';

		if($event_recurrence['type'] === 'once' || $event_recurrence['type'] === 'daily' || $event_recurrence['type'] === 'custom')
			$repeat = _n('day', 'days', $event_recurrence['repeat'], 'cdash-events');
		elseif($event_recurrence['type'] === 'weekly')
			$repeat = _n('week', 'weeks', $event_recurrence['repeat'], 'cdash-events');
		elseif($event_recurrence['type'] === 'monthly')
			$repeat = _n('month', 'months', $event_recurrence['repeat'], 'cdash-events');
		else
			$repeat = _n('year', 'years', $event_recurrence['repeat'], 'cdash-events');
		...
		else
		{
			$html .= '
				<div class="event-custom">
					<label for="event_separate_end_date_1">'.__('Separate end date', 'cdash-events').'</label><input class="event_separate" id="event_separate_end_date_1" type="checkbox" name="event_recurrence[custom][separate_end_date][1]"/>
					<div class="start">
						<label for="event_start_date_1">'.__('Start date/time', 'cdash-events').':</label> <input id="event_start_date_1" type="text" name="event_recurrence[custom][dates][1][start][date]" value="" class="event_custom_date"/> <input id="event_start_time_0" type="text" name="event_recurrence[custom][dates][1][start][time]" value="" class="event_custom_time"/>
						<a class="delete-custom-event button button-secondary" href="#">'.__('Delete', 'cdash-events').'</a>
					</div>
					<div class="end" style="display: none">
						<label for="event_end_date_1">'.__('End date/time', 'cdash-events').':</label> <input id="event_end_date_1" type="text" name="event_recurrence[custom][dates][1][end][date]" value="" class="event_custom_date"/> <input id="event_end_time_0" type="text" name="event_recurrence[custom][dates][1][end][time]" value="" class="event_custom_time"/>
					</div>
				</div>';
		}
		...
		do_action('cde_before_metabox_event_datetime');

		echo $html;

		do_action('cde_after_metabox_event_datetime');
	}
	...
	*/
	...
	{
		wp_nonce_field('cdash_events_save_event_tickets', 'event_nonce_tickets');

		$tickets = get_post_meta($post->ID, '_event_tickets', true);
		$free_event = (($free = get_post_meta($post->ID, '_event_free', true)) === '' ? '1' : $free);
		$html_t = '';
		$symbol = cde_get_currency_symbol();

		$html = '
		<p>
			<label for="event_free">'.__('Is this a free event?', 'cdash-events').'</label>
		...
		<div id="event_cost_and_tickets"'.($free_event === '1' ? ' style="display: none;"' : '').'>
			<div>
				<a href="#" id="event_add_ticket" class="button button-primary">'.__('Add new ticket', 'cdash-events').'</a>
			</div>';
		...
		if(!empty($tickets) && is_array($tickets))
		{
			foreach($tickets as $id => $ticket)
			{
				...
				$html_t .= '
					<a href="#" class="event_ticket_delete button button-secondary">'.__('Delete', 'cdash-events').'</a>
				</p>';
			}
		}
		else
		...
		$html .= '
			...
			<div>
				<label for="event_tickets_url">'.__('Buy tickets URL', 'cdash-events').':</label> <input id="event_tickets_url" class="regular-text" type="text" name="event_tickets_url" value="'.esc_url(get_post_meta($post->ID, '_event_tickets_url', true)).'" />
			</div>
		</div>';
		
		do_action('cde_before_metabox_event_tickets');

		echo $html;

		do_action('cde_after_metabox_event_tickets');
	...
	*/
	...
	{
		// security nonce
		wp_nonce_field('cdash_events_save_event_options', 'event_nonce_options');
		...
		$options = apply_filters('cde_metabox_event_display_options', $options, $post);
		$values = apply_filters('cde_metabox_event_display_values', get_post_meta($post->ID, '_event_display_options', true), $post);
		
		// display options
		do_action('cde_before_metabox_event_display_options', $post);
		...
		do_action('cde_after_metabox_event_display_options', $post);
	}
	...
	*/
	public function save_event($post_ID)
	{
		...
		if(isset($_POST['event_nonce_datetime']) && wp_verify_nonce($_POST['event_nonce_datetime'], 'cdash_events_save_event_datetime'))
		{
			...
			if($event_all_day === 1)
			{
				if($cde_helper->is_valid_date($_POST['event_start_date']))
				{
					$start_date_ok = true;
				...
				if($cde_helper->is_valid_date($_POST['event_end_date']))
				{
					if($start_date_ok)
					{
						if($cde_helper->is_after_date($_POST['event_end_date'], $_POST['event_start_date']))
						{
			...
			{
				if($cde_helper->is_valid_date($_POST['event_start_date']) && $cde_helper->is_valid_time($_POST['event_start_time']))
				{
					$start_date_ok = true;
				...
				if($cde_helper->is_valid_date($_POST['event_end_date']) && $cde_helper->is_valid_time($_POST['event_end_time']))
				{
					if($start_date_ok)
					{
						if($cde_helper->is_after_date($_POST['event_end_date'].' '.$_POST['event_end_time'], $_POST['event_start_date'].' '.$_POST['event_start_time']))
						{
		...
		if(isset($_POST['event_nonce_tickets']) && wp_verify_nonce($_POST['event_nonce_tickets'], 'cdash_events_save_event_tickets'))
		{
		...
		if(isset($_POST['event_nonce_options']) && wp_verify_nonce($_POST['event_nonce_options'], 'cdash_events_save_event_options'))
		{
			...
			$event_display_options = apply_filters('cde_metabox_event_display_options', $event_display_options, $post_ID);
	...
	private function add_custom_dates($post_id, $dates, $all_day, $separate_end_date, $start, $end)
	{
		...
		if($all_day === 1)
		{
			foreach($dates as $id => $date)
			{
				...
				{
					if($cde_helper->is_valid_date($date['start']['date']) && $cde_helper->is_valid_date($date['end']['date']) && $cde_helper->is_after_date($date['end']['date'], $date['start']['date'], false))
					{
						$separates[] = true;
						$custom_dates[] = array(
				...
				{
					if($cde_helper->is_valid_date($date['start']['date']))
					{
						$separates[] = false;
						$custom_dates[] = array(
							'start' => $date['start']['date'].' 00:00:00',
		...
		else
		{
			foreach($dates as $id => $date)
			{
				if(isset($separate_end_date[$id]))
				{
					if($cde_helper->is_valid_date($date['start']['date']) && $cde_helper->is_valid_date($date['end']['date']) && $cde_helper->is_valid_time($date['start']['time']) && $cde_helper->is_valid_time($date['end']['time']) && $cde_helper->is_after_date($date['end']['date'].' '.$date['end']['time'], $date['start']['date'].' '.$date['start']['time'], false))
					{
						$separates[] = true;
						$custom_dates[] = array(
				...
				{
					if($cde_helper->is_valid_date($date['start']['date']) && $cde_helper->is_valid_time($date['start']['time']))
					{
	...
	*/
	private function create_recurrences($post_id, $start, $end, $type, $repeat, $until, $weekly_days, $monthly_day_type)
	{
		...
		if($cde_helper->is_after_date($start, $end) || $cde_helper->is_after_date($start, $until))
			return;
